...
def central_difference(f: Any, *vals: Any, arg: int = 0, epsilon: float = 1e-6) -> Any:
    ...
    return (f(*vals[:arg], vals[arg] + epsilon, *vals[arg + 1:]) - f(*vals[:arg], vals[arg] - epsilon, *vals[arg + 1:])) / (epsilon * 2)
...
def backpropagate(variable: Variable, deriv: Any) -> None:
    ...
    for var in topo:
        if var.is_leaf():
            ...
            tmp = var.chain_rule(derivatives[var.unique_id])
            for par, bw in tmp:
                if par.unique_id not in derivatives:
                    derivatives[par.unique_id] = 0.
                derivatives[par.unique_id] += bw
